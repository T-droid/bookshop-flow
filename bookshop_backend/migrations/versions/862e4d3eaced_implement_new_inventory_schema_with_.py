"""implement_new_inventory_schema_with_correct_keys

Revision ID: 862e4d3eaced
Revises: 8261c503b92c
Create Date: 2025-08-13 16:53:19.586353
...
def upgrade() -> None:
    ...
    op.add_column('purchaseorder', sa.Column('po_id', sa.Uuid(), nullable=False))
    ...
    op.drop_constraint(op.f('purchaseorder_supplier_id_fkey'), 'purchaseorder', type_='foreignkey')
    ...
    op.create_foreign_key(None, 'purchaseorderitems', 'purchaseorder', ['order_id'], ['po_id'], ondelete='CASCADE')
    op.add_column('supplier', sa.Column('supplier_id', sa.Uuid(), nullable=False))
    ...
    op.drop_constraint(op.f('tenantsupplier_supplier_id_fkey'), 'tenantsupplier', type_='foreignkey')
    op.create_foreign_key(None, 'tenantsupplier', 'supplier', ['supplier_id'], ['supplier_id'], ondelete='CASCADE')
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'tenantsupplier', type_='foreignkey')
    op.create_foreign_key(op.f('tenantsupplier_supplier_id_fkey'), 'tenantsupplier', 'supplier', ['supplier_id'], ['id'], ondelete='CASCADE')
    op.add_column('supplier', sa.Column('id', sa.UUID(), autoincrement=False, nullable=False))
    op.drop_index(op.f('ix_supplier_name'), table_name='supplier')
    ...
    op.alter_column('supplier', 'name',
               ...
               existing_nullable=False)
    op.drop_column('supplier', 'supplier_id')
    ...
    op.add_column('purchaseorder', sa.Column('id', sa.UUID(), autoincrement=False, nullable=False))
    op.drop_constraint(None, 'purchaseorder', type_='foreignkey')
    op.drop_constraint(None, 'purchaseorder', type_='foreignkey')
    op.create_foreign_key(op.f('purchaseorder_supplier_id_fkey'), 'purchaseorder', 'supplier', ['supplier_id'], ['id'])
    ...
    op.drop_column('purchaseorder', 'po_id')
